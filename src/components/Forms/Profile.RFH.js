...
const Profile = ({ editProfile }) => {
  ...
  const mapProfileDataToForm = useCallback(
    (data) => {
      ...
      const missingValues = Object.keys(profile).filter(
        (key) => !Object.keys(data).includes(key),
      );
      // console.log('missingValues: ', missingValues)
      const errorKeys = missingValues.reduce(
        (acc, cv) => ({
          ...acc,
          [cv]: {
            hasError: true,
  ...
  const onErrors = (errors) => {
    ...
    const errorKeys = Object.keys(errors).reduce(
      (acc, cv) => ({
        ...acc,
        [cv]: { hasError: true, msg: `${formatErrorStr(cv)} is required!` },
      }),
      {},
    );
    setErrors((e) => ({ ...e, ...errorKeys }));
